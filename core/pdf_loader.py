...
VISION_MAX_PAGES   = 5   # 이미지 분석 포함 모드에서 처리할 최대 페이지 수
                         # None으로 바꾸면 전체 페이지 처리 (비용 주의)
...
def _process_page(pdf_path: str, name: str, page_index: int, cancel_check) -> dict | None:
    ...
    try:
        ...
        return {"page_num": page_num, "mode": mode,
                "chunk": {"content": content, "source": source}}

    except Exception as e:
        print(f"  ⚠️ {page_num}페이지 {mode} 실패: {e}", file=sys.stderr)
        # 최후 폴백: 텍스트 직접 추출
        if info["text"]:
            return {"page_num": page_num, "mode": "text",
                    "chunk": {"content": info["text"],
                              "source": f"{name} - {page_num}페이지"}}
        return None
...
def load_pdf(pdf_path: str, filename: str = None, vision: bool = False,
    ...
    return [e["chunk"] for e in stream_pdf(pdf_path, filename, vision, cancel_check)
            if e["type"] == "chunk"]
...
def stream_pdf(pdf_path: str, filename: str = None, vision: bool = False,
    ...
    if not vision:
        ...
        for i in range(total_pages):
            ...
            if content:
                processed += 1
                yield {"type": "chunk",
                       "chunk": {"content": content, "source": f"{name} - {i+1}페이지"},
                       "page": i + 1, "total": total_pages, "mode": "text"}
        yield {"type": "done", "total": processed}
        return

    # ── 이미지 분석 포함 모드 ─────────────────────────────────────
    # VISION_MAX_PAGES로 처리 페이지 수 제한 (비용/시간 조절용)
    limit = min(total_pages, VISION_MAX_PAGES) if VISION_MAX_PAGES else total_pages
    if limit < total_pages:
        print(f"  [Vision] 처음 {limit}페이지만 처리 (전체 {total_pages}페이지)",
              file=sys.stderr)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=VISION_MAX_WORKERS) as executor:
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            if result:
                processed += 1
                yield {"type": "chunk", "chunk": result["chunk"],
                       "page": result["page_num"], "total": limit,
                       "mode": result["mode"]}
